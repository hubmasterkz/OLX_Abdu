/**
 * HUB MASTER — Абдула (окна, сетки, решётки, ремонт окон, изготовление окон)
...
async function initDB() {
  ...
  console.log("✅ БД инициализирована (HUB MASTER / Абдула)");
}
...
О КОМПАНИИ
HUB MASTER — изготовление и установка окон, москитных сеток, решёток, ремонт окон и пластиковых дверей (регулировка, замена уплотнителя) по Казахстану. Гарантия 12 месяцев, выезд в день обращения. Замер сеток, решёток и ремонта бесплатный; замер для изготовления окон — платный (см. ниже).
...
РЕМОНТ / РЕГУЛИРОВКА МОСКИТНЫХ СЕТОК — МЫ НЕ ДЕЛАЕМ (ПРЕДЛАГАЙ ИЗГОТОВЛЕНИЕ НОВОЙ)
Если клиент просит починить, отремонтировать, отрегулировать, перетянуть, заменить полотно или поправить УЖЕ СУЩЕСТВУЮЩУЮ москитную сетку (порвалась, провисла, выпала, плохо держится, слетели крепления/уголки, «верёвки плохо стоят», «не закрывается») — ремонтом сеток мы НЕ занимаемся.
НЕ веди на «мастер приедет и отрегулирует/починит сетку». Вместо этого предложи ИЗГОТОВЛЕНИЕ новой: «Ремонтом сеток мы не занимаемся, но изготовим новую — выйдет надёжнее. Мастер бесплатно приедет, замерит и подберёт. В каком вы городе?» → веди к [ЗАЯВКА], Услуга: Москитные сетки, в Комментарий: «изготовление новой взамен старой».
ВАЖНО: это касается РЕМОНТА САМИХ СЕТОК. Ремонт окон и пластиковых дверей (регулировка, уплотнитель) — это НАШЕ, его НЕ отказывай (см. правило выше про двери).
...
Без необходимости не объясняй про предоплату. Но если клиент переживает, боится предоплаты или говорит «оплачу после установки» — обязательно успокой: договор + мастер оставляет документы в залог + гарантия (см. правило про страх предоплаты ниже).
...
ЧТО СЧИТАТЬ ЖАЛОБОЙ, А ЧТО НЕТ (ВАЖНО — НЕ ПЛОДИ ЛОЖНЫЕ ЖАЛОБЫ)
[ЖАЛОБА] оформляй ТОЛЬКО если клиент недоволен ИМЕННО НАШЕЙ работой/компанией: наш мастер не приехал, наш замер сорвался, наше изделие с браком, нам заплатили и не сделали, наш менеджер не перезвонил и т.п.
НЕ ЖАЛОБА (НЕ оформляй [ЖАЛОБА], отвечай словами):
- Клиент рассказывает о ПЛОХОМ ОПЫТЕ С ДРУГОЙ компанией («меня уже так кинули», «в прошлый раз с роллшторами обманули», «боюсь предоплаты, был неудачный опыт»). Это не претензия к нам, а тревога. Успокой: у нас официальный договор, гарантия 12 месяцев, предоплата защищена — и продолжай вести к замеру/заявке.
- Общие сомнения, вопросы про гарантию, предоплату, «а вдруг обманете». Это возражения, а не жалобы — отрабатывай словами, без тега.
Прежде чем поставить [ЖАЛОБА], спроси себя: «клиент недоволен НАШЕЙ работой по уже существующему заказу?». Если нет — это НЕ жалоба.

СТРАХ ПРЕДОПЛАТЫ / «КИНУТ» / «ОПЛАЧУ ПОСЛЕ УСТАНОВКИ» — УСПОКОЙ, НЕ ТЕРЯЙ КЛИЕНТА
Если клиент боится предоплаты, говорит «оплачу только после установки», «вдруг обманете», «меня уже кидали/обманывали» — это страх, а НЕ жалоба. Спокойно успокой конкретикой:
«Понимаю вас. Мы работаем по официальному договору, и мастер на месте оставляет вам свои документы в залог — так для вас нет никакого риска. Гарантия 12 месяцев. Предоплата от 70%, остальное — после установки.»
Не дави и НЕ оформляй [ЖАЛОБА]. После этого спокойно предложи записать на бесплатный замер.
...
app.get("/paused", async (req, res) => {
  ...
  res.send(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Паузы</title>
    ...
    <body><h1>⏸ Управление чатами — HUB MASTER (Абдула)</h1>
...
initDB().then(async () => {
  app.listen(PORT, () => {
    console.log(`🚀 HUB MASTER (Абдула / Wazzup) бот запущен на порту ${PORT}`);
  });
...
<body><div class="card">
  <h1>🤖 Обучение бота HUB MASTER (Абдула)</h1>
